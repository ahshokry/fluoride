...
static int skt_connect(const char* path, size_t buffer_sz) {
  ...
  LOG(INFO) << "SOCKET FD " << skt_fd;
  err = osi_socket_local_client_connect(
          skt_fd, path, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM);
  if (err < 0) {
	LOG(ERROR) << "error of disconnection= " << err;
...
static int adev_open(const hw_module_t* module, const char* name,
  ...
  LOG(INFO) << " adev_open in ha_hw module";
  FNLOG();
