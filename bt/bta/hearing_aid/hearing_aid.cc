...
g722_decode_state_t* decoder = nullptr;
...
//    BTM_SecBondByTransport(address, GATT_TRANSPORT_LE, 0, NULL, 0);
  }
...
//    }
  ...
  void StartSendingAudio(const HearingDevice& hearingDevice) {
    ...
    if (encoder_state_left == nullptr) {
      ...
      decoder = g722_decode_init(nullptr, 64000, G722_PACKED);
...
//      LOG(FATAL) << "num_samples is not even: " << num_samples;
    ...
    if (left == nullptr || right == nullptr) {
      for (int i = 0; i < num_samples; i++) {
        const uint8_t* sample = data.data() + i * 2;
//		printf("element is %d\n",*sample);

        int16_t left = (int16_t)((*(sample + 1) << 8) + *sample);//
//        sample += 2;
//		printf("element is %d\n",*sample);
//        int16_t right = (int16_t)((*(sample + 1) << 8) + *sample)>> 1;//
//        uint16_t mono_data = (int16_t)(((uint32_t)left + (uint32_t)right) )>> 1;//
//        if(!one_time)
//        LOG(INFO) << "left data " << (int)left;
...
////	if( before_encode == NULL )
//	   {
//	       printf("Could not open file.");
//	   }
//	   else
//	   {
//		   size_t written_Data = fwrite( (const char*)chan_left.data(), 1, 640, before_encode );
////			       fclose( fid );
//	       printf("Wrote data to 'before_encode.raw %d'\n",written_Data);
//	   }
...
//      LOG(INFO) << "chan_left.size() "<<chan_left.size() ;

//      LOG(INFO) << "encoder_state_left.itu_test_mode"<<encoder_state_left->itu_test_mode;
...
//      LOG(INFO) << "data "<<(uint16_t)encoded_data_left.data()[0] ;
//      LOG(INFO) << "data "<<(uint16_t)encoded_data_left.data()[1] ;
//      LOG(INFO) << "data "<<(uint16_t)encoded_data_left.data()[2] ;
//      LOG(INFO) << "data "<<(uint16_t)encoded_data_left.data()[3] ;


      // chan_left.size() instead of null last param
//      LOG(INFO) << "encoded size: "<<encoded_size ;
//      if(encoded_size == 0 ){
//    	  LOG(INFO) << "no encoded size";
//    	  return;
//      }
//
//		if( fid == NULL )
...
//		   else
//		   {
//			   size_t written_Data = fwrite( (const char*)encoded_data_left.data(), 2, (int)encoded_size/2, fid );
////			       fclose( fid );
//		       printf("Wrote data to 'encoded.raw %d'\n",written_Data);
//		   }
//		int deoded_size =
//		          g722_decode(decoder,decoded_data,(const uint8_t*)encoded_data_left.data(),160,4096*2);
//
//		if( fid_decoder == NULL )
//		   {
//		       printf("Could not open file.");
...
//		   else
//		   {
//			   size_t written_Data = fwrite( (const char*)decoded_data, 1, (int)640, fid_decoder );
////			       fclose( fid );
//		       printf("Wrote data to 'decoded.raw %d'\n",written_Data);
//		   }
      ...
      if (packets_to_flush) {
        VLOG(0) << left->address << " skipping " << packets_to_flush
                << " packets";
    ...
    if (right) {
      ...
      uint16_t packets_to_flush = L2CA_FlushChannel(cid, L2CAP_FLUSH_CHANS_GET);
      if (packets_to_flush) {
        VLOG(0) << right->address << " skipping " << packets_to_flush
                << " packets";
...
//    LOG(INFO) << " PACKET SIZE = " << 	packet_size << " codec_in_use " <<(int)codec_in_use <<
//    		" default_data_interval_ms " <<default_data_interval_ms;
    for (size_t i = 0; i < 1; i += packet_size) {
      if (left) {
        left->audio_stats.packet_send_count++;
        SendAudio(encoded_data_left.data() + i, packet_size, left); //this was packet sizw
      }
      if (right) {
...
//	  one_time = 1;
    ...
    BT_HDR* audio_packet = malloc_l2cap_buf(packet_size);
...
void HearingAid::Initialize(
  ...
  fid = fopen("encoded.raw", "wb");
  fid_decoder =fopen("decoded.raw", "wb");
  before_encode=fopen("before_encode.raw", "wb");
