...
alarm_t* audio_timer_wait = nullptr;
...
//  VLOG(0) << "bytes_per_tick " << bytes_per_tick << " num_channels = " <<num_channels;
//  VLOG(0) << "sample_rate " << sample_rate << " data_interval_ms = " <<data_interval_ms;
//  VLOG(0) << "bit_rate " << bit_rate ;
...
void send_audio_data_wait(void*){
    audio_timer = alarm_new_periodic("hearing_aid_data_timer");
    LOG(INFO)<< "FIRE ONE TIME";
    alarm_set_on_mloop(audio_timer, 20, send_audio_data, //this send successfully with interval 25 msec but when interval 20 so many packets dropped
                       nullptr);
}
...
//      audio_timer_wait = alarm_new("wait a time to fire the data timer");
//      alarm_set_on_mloop(audio_timer_wait, 100, send_audio_data_wait,
//                         nullptr);
      audio_timer = alarm_new_periodic("hearing_aid_data_timer");
      alarm_set_on_mloop(audio_timer, 20, send_audio_data, //this send successfully with interval 25 msec but when interval 20 so many packets dropped
                         nullptr);
...
//  VLOG(0) << "Hearing Aid UIPC Open" << codecConfiguration.bit_rate <<codecConfiguration.sample_rate << codecConfiguration.data_interval_ms;
